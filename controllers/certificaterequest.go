...
func (r *CertificateRequestReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if !ok {
		...
		_ = r.setStatus(ctx, cr, cmmeta.ConditionFalse, certmanager.CertificateRequestReasonPending, fmt.Sprintf("Failed to load provisioner for StepIssuer resource %s", issNamespaceName))
		return reconcile.Result{}, err
	}
...
// setStatus is a helper function to set the CertificateRequest status condition with reason and message, and update the API.
...
// issuerHasCondition will return true if the given StepIssuer resource has
