...
func (r *IssuerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !ok {
		err := fmt.Errorf("secret %s does not contain key %q", passwordSecret.Name, iss.Spec.User.Key)
		log.Error(err, "failed to retrieve Issuer auth secret")
		_ = r.setStatus(ctx, iss, api.ConditionFalse, "NotFound", fmt.Sprintf("Failed to retrieve auth secret: %v", err))
	...
	p, err := provisioners.New(iss, string(user), string(password))
	if err != nil {
	...
	return ctrl.Result{}, r.setStatus(ctx, iss, api.ConditionTrue, "Verified", "Issuer verified and ready to sign certificates")
}
